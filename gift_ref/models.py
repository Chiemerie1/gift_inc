...
class ReferralLink(models.Model):
	identifier = models.CharField(max_length=50, blank=True, unique=True,default=uuid4)
	user = models.OneToOneField(
		settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="referal_link"
	)
	...
	def get_absolute_url(self):
		return reverse("gift_ref_reflink", kwargs={"identifier": self.identifier})


class ReferralHit(models.Model):
	id = models.UUIDField(primary_key=True, default=uuid4, editable=False)
	referral_link = models.ForeignKey(ReferralLink, on_delete=models.CASCADE,related_name='referrals')
